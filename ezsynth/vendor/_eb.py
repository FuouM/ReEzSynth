...
class EbsynthRunner:
    ...
    def validate_guides(
        ...
        for i in range(len(guides)):
            ...
            if s_h != sh or s_w != sw:
                error_msg = (
                    f"Guide source and style resolution must match. "
                    f"Style is ({sw}x{sh}), but guide source is ({s_w}x{s_h}). "
                    f"Tip: Check if your content frames and style image have the same resolution. "
                    f"You can enable 'force_style_size: true' in your project config to fix this automatically."
                )
