...
from ezsynth.vendor.raft.core.core.utils.utils import coords_grid
...
class FlowDiffuser(nn.Module):
    ...
    def up_sample_flow8(self, flow, mask):
        B, _, H, W = flow.shape
        flow = torch.nn.functional.unfold(
            8 * flow, kernel_size=[3, 3], stride=[1, 1], padding=[1, 1]
        )
    ...
    def up_sample_flow4(self, flow, mask):
        B, _, H, W = flow.shape
        flow = torch.nn.functional.unfold(
            4 * flow, kernel_size=[3, 3], stride=[1, 1], padding=[1, 1]
        )
        flow = flow.reshape(B, 2, 9, 1, 1, H, W)
    ...
    def initialize_flow8(self, img):
        ...
        N, C, H, W = img.shape
        coords0 = (
            coords_grid(N, H // 8, W // 8, device=img.device)
            .permute(0, 2, 3, 1)
            .contiguous()
        )
        coords1 = (
    ...
    def initialize_flow4(self, img):
        ...
        N, C, H, W = img.shape
        coords0 = (
            coords_grid(N, H // 4, W // 4, device=img.device)
            .permute(0, 2, 3, 1)
            .contiguous()
        )
        coords1 = (
    ...
    def _train_dfm(self, feat_shape, flow_gt, net, inp8, coords0, coords1):
        ...
        flow_gt_sp8 = (
            F.interpolate(flow_gt, (h, w), mode="bilinear", align_corners=True) / 8.0
        )
        ...
        for ii in range(self.recurr_itrs):
            ...
            with torch.amp.autocast("cuda"):
                ...
                net, delta_flow = self.update_dfm(
                    net,
                    inp8,
                    corr,
                    flow,
    ...
    @torch.no_grad()
    def _ddim_sample(
        self, feat_shape, net, inp, coords0, coords1_init, clip_denoised=True
    ):
        ...
        for i_ddim, time_s in enumerate(time_pairs):
            ...
            t_next = torch.full(
                (batch,), time_next, device=self.device, dtype=torch.long
            )
    ...
    def _model_predictions(
        self, x, t, net, inp8, coords0, coords1, i_ddim, pred_last=None, t_next=None
    ):
    ...
    def forward(
        self, image1, image2, test_mode=False, iters=None, flow_gt=None, flow_init=None
    ):
        ...
        if self.diffusion:
            ...
            h, w = fmap1_8.shape[-2:]
            self.norm_const = torch.as_tensor(
                [w, h], dtype=torch.float, device=self.device
            ).view(1, 2, 1, 1)

            if self.training:
                coords1 = coords1.detach()
                flow_up_s, coords1, net = self._train_dfm(
                    fmap1_8.shape, flow_gt, net, inp8, coords0, coords1
                )
            else:
                coords1, net, flow_up_s = self._ddim_sample(
                    fmap1_8.shape, net, inp8, coords0, coords1
                )
            ...
            if self.sp4:
                ...
                net = torch.nn.functional.interpolate(
                    net, scale_factor=2, mode="bilinear", align_corners=True
                )
                coords1_rd = ste_round(coords1)

                corr_fn4 = CorrBlock_FD_Sp4(
                    fmap1_4,
                    fmap2_4,
                ...
                for itr in range(self.args.iters_const6):
                    ...
                    flow = coords1 - coords0
                    with torch.amp.autocast("cuda"):
                        net, delta_flow = self.update(
                            net, inp4, corr, flow, itr, sp4=True
                        )
