...
from ezsynth.vendor.raft.core.core.corr import CorrBlock
...
class TransformerModule(nn.Module):
    ...
    def forward(self, fmap1, fmap2, inp):
        ...
        corr_fn = CorrBlock(
            fmap1, fmap2, num_levels=self.args.corr_levels, radius=self.args.corr_radius
        )
        return corr_fn
...
def window_partition(fmap, window_size):
    ...
    fmap = fmap.reshape(
        B * (H // window_size[0]) * (W // window_size[1]),
...
class GlobalAttention(nn.Module):
    ...
    def forward(self, fmap1, fmap2, mask=None):
        ...
        attn = q @ k.transpose(1, 2)
        if mask is not None:
            mask = mask.reshape(1, H * W, 1) | mask.reshape(
                1, 1, H * W
            )  # batch, hw, hw
...
class SelfTransformerBlcok(nn.Module):
    def __init__(self, dim, mlp_scale, window_size, shift_size=None, norm=None):
        ...
        self.mlp = nn.Sequential(
            nn.Linear(dim, dim * mlp_scale), nn.GELU(), nn.Linear(dim * mlp_scale, dim)
        )
    ...
    def forward(self, fmap, mask=None):
        ...
        if self.shift_size is not None:
            shifted_fmap = torch.roll(
                fmap, [-self.shift_size[0], -self.shift_size[1]], dims=(1, 2)
            )
            if mask is not None:
                shifted_mask = torch.roll(
                    mask, [-self.shift_size[0], -self.shift_size[1]], dims=(1, 2)
                )
        else:
            shifted_fmap = fmap
            if mask is not None:
        ...
        if self.shift_size is not None:
            ...
            win_mask = win_mask.reshape(
                -1, self.window_size[0] * self.window_size[1]
            )  # nW, Wh*Ww
            attn_mask = win_mask.unsqueeze(2) - win_mask.unsqueeze(
                1
            )  # nw, Wh*Ww, Wh*Ww
            if mask is not None:
                attn_mask = attn_mask.masked_fill(attn_mask == 0, 0.0).masked_fill(
                    (attn_mask != 0) | pad_mask, -100.0
                )
            else:
                attn_mask = attn_mask.masked_fill(attn_mask == 0, 0.0).masked_fill(
                    attn_mask != 0, -100.0
            ...
            if mask is not None:
                ...
                pad_mask = pad_mask.masked_fill(pad_mask != 0, -100.0).masked_fill(
                    pad_mask == 0, 0.0
                )
        ...
        if self.shift_size is not None:
            fmap = torch.roll(
                shifted_fmap, [self.shift_size[0], self.shift_size[1]], dims=(1, 2)
            )
        else:
            fmap = shifted_fmap
...
class BasicLayer(nn.Module):
    def __init__(self, num_layer, dim, mlp_scale, window_size, cross=False):
        ...
        for n in range(num_layer):
            shift_size = (
                None if n % 2 == 0 else [window_size[0] // 2, window_size[1] // 2]
            )
            self.blocks.append(
        ...
        if cross:
            self.cross_transformer = CrossTransformerBlcok(
                dim=dim, mlp_scale=mlp_scale, norm="layer"
            )
...
class UpSampleMask8(nn.Module):
    def __init__(self, dim):
        ...
        self.up_sample_mask = nn.Sequential(
            nn.Conv2d(
                in_channels=dim, out_channels=256, kernel_size=3, padding=1, stride=1
            ),
...
class Aggregator(nn.Module):
    ...
    def forward(self, *inputs):
        ...
        if itr == 0:
            ...
            self.attn = (
                attn.softmax(2).view(b, h * w, h * w).permute(0, 2, 1).contiguous()
            )

        feat_v = self.to_v(feat_mo).view(b, c, h * w)
        feat_o = (
            torch.einsum("b n m, b c m -> b c n", self.attn, feat_v)
            .contiguous()
            .view(b, c, h, w)
        )
...
class SKUpdate(nn.Module):
    def __init__(self, args, hidden_dim):
        ...
        self.gru = PCBlock4_Deep_nopool_res(
            128 + hidden_dim + hidden_dim + 128, d_dim, k_conv=args.PCUpdater_conv
        )
...
class ConvEE(nn.Module):
    def __init__(self, C_in, C_out):
        ...
        self.conv2 = nn.Sequential(nn.GELU(), nn.Conv2d(C_in, C_in, 3, padding=1))
        self.gamma = nn.Parameter(torch.zeros(1))
