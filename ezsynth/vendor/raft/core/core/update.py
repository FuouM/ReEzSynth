...
class LookupScaler(nn.Module):
    def __init__(
        ...
        self.model_scale = nn.Sequential(
            nn.Linear(4 * input_dim, (output_dim // 2) * output_size), nn.Sigmoid()
        )
        self.model_add = nn.Sequential(
            nn.Linear(4 * input_dim, (output_dim // 2) * output_size), nn.Sigmoid()
        )
    ...
    def forward(self, context_map, hidden_state):
        ...
        convert_map = self.convert_conv(
            torch.cat([context_map, hidden_state], dim=1).type(torch.float32)
        )
        lookup_context = torch.cat(
            [torch.amax(convert_map, dim=(2, 3)), torch.amin(convert_map, dim=(2, 3))],
            dim=-1,
        ).type(torch.float32)
        scale = (
            self.model_scale(lookup_context).view(
                -1, self.output_size, self.output_dim // 2
            )
            * self.max_multiplier
            + 1
        )
        add = (
            self.model_add(lookup_context).view(
                -1, self.output_size, self.output_dim // 2
            )
