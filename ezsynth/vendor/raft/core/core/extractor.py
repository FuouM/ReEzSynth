...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, num_hiddens, dropout=0.0, max_len=10000):
        ...
        X = torch.arange(1, max_len + 1, dtype=torch.float32).reshape(
            -1, 1
        ) / torch.pow(
...
class CoordinateAttention(nn.Module):
    def __init__(self, feature_size=128, enc_size=128, heads=4, bias=True, dropout=0.0):
        ...
        self.att = nn.MultiheadAttention(
            feature_size, heads, dropout=dropout, bias=bias, batch_first=True
        )
    ...
    def forward(self, x):
        ...
        col_res = col_res.reshape(c_shape[0], c_shape[2], c_shape[1], c_shape[3])[
            :, :, :, : self.enc_size
        ]
