# ezsynth/engines/backends/taichi_ops.py
import numpy as np
import taichi as ti
...
@ti.data_oriented
class TaichiOps:
    """Utility class for Taichi-accelerated image operations."""

    # --- Color Conversion Kernels ---

    @ti.func
    def bgr_to_lab_func(self, bgr):
        # Normalize to [0, 1]
        b = bgr[0] / 255.0
        g = bgr[1] / 255.0
        r = bgr[2] / 255.0
    ...
    def compute_stats_kernel(
        self, img: ti.types.ndarray(), mean: ti.types.ndarray(), std: ti.types.ndarray()
    ):
    ...
    def hist_blend_apply_kernel(
        ...
        for i, j in ti.ndrange(h, w):
            ...
            lab_a = self.bgr_to_lab_func(bgr_a)
            lab_b = self.bgr_to_lab_func(bgr_b)
    ...
    def hist_blend_final_pass_kernel(
        ...
        for i, j in ti.ndrange(h, w):
            ...
            res = self.lab_to_bgr_func(lab_final)
            out[i, j, 0] = res[0]
            out[i, j, 1] = res[1]
            out[i, j, 2] = res[2]
    ...
    @ti.func
    def get_val_reflect(
        self, src: ti.template(), y: int, x: int, c: int, h: int, w: int
    ):
        ry = y
        rx = x
        if ry < 0:
    ...
    def bilinear_warp_kernel(
        self, src: ti.types.ndarray(), flow: ti.types.ndarray(), dst: ti.types.ndarray()
    ):
        ...
        for i, j in ti.ndrange(h, w):
            ...
            if ti.static(len(src.shape) > 2):
                for ch in range(src.shape[2]):
                    v00 = self.get_val_reflect(src, y0, x0, ch, h, w)
                    v01 = self.get_val_reflect(src, y0, x1, ch, h, w)
                    v10 = self.get_val_reflect(src, y1, x0, ch, h, w)
                    v11 = self.get_val_reflect(src, y1, x1, ch, h, w)
                    dst[i, j, ch] = wy0 * (wx0 * v00 + wx1 * v01) + wy1 * (
                        wx0 * v10 + wx1 * v11
                    )
            else:
                v00 = self.get_val_reflect(src, y0, x0, 0, h, w)
                v01 = self.get_val_reflect(src, y0, x1, 0, h, w)
                v10 = self.get_val_reflect(src, y1, x0, 0, h, w)
                v11 = self.get_val_reflect(src, y1, x1, 0, h, w)
    ...
    def compute_laplacian_kernel(
        self, x: ti.types.ndarray(), lap: ti.types.ndarray(), weight_sq: float
    ):
    ...
    def poisson_solver_cg(self, gx, gy, target, weight, max_iter=100, tol=1e-5):
        h, w = target.shape
        x = np.zeros_like(target)
        weight_sq = weight * weight
        rhs = np.zeros_like(target)
        self.compute_rhs_kernel(gx, gy, target, rhs, weight_sq)

        r = np.zeros_like(target)
        Ap = np.zeros_like(target)
        self.compute_laplacian_kernel(x, Ap, weight_sq)
        r = rhs - Ap
        p = r.copy()
        rsold = np.sum(r * r)
        ...
        for i in range(max_iter):
            self.compute_laplacian_kernel(p, Ap, weight_sq)
