...
from . import backbone_v7, config, corr, matching, refine, transformer, upsample

try:
    from huggingface_hub import PyTorchModelHubMixin
except ImportError:
    # A simple fallback if huggingface_hub is not installed.
    # This allows the model to be used locally without the dependency.
...
class NeuFlow(torch.nn.Module, PyTorchModelHubMixin):
    def __init__(self):
        ...
        self.backbone = backbone_v7.CNNEncoder(
            config.feature_dim_s16,
            config.context_dim_s16,
            config.feature_dim_s8,
            config.context_dim_s8,
        )

        self.cross_attn_s16 = transformer.FeatureAttention(
            config.feature_dim_s16 + config.context_dim_s16,
            num_layers=2,
            ffn=True,
            ffn_dim_expansion=1,
            post_norm=True,
        ...
        self.merge_s8 = torch.nn.Sequential(
            torch.nn.Conv2d(
                config.feature_dim_s16 + config.feature_dim_s8,
                config.feature_dim_s8,
                kernel_size=3,
                stride=1,
                padding=1,
        ...
        self.context_merge_s8 = torch.nn.Sequential(
            torch.nn.Conv2d(
                config.context_dim_s16 + config.context_dim_s8,
                config.context_dim_s8,
                kernel_size=3,
                stride=1,
                padding=1,
        ...
        self.conv_s8 = backbone_v7.ConvBlock(
            3, config.feature_dim_s1, kernel_size=8, stride=8, padding=0
        )
    ...
    def forward(self, img0, img1, iters_s16=1, iters_s8=8):
        ...
        features_s16, context_s16 = self.split_features(
            features_s16, config.context_dim_s16, config.feature_dim_s16
        )
        features_s8, context_s8 = self.split_features(
            features_s8, config.context_dim_s8, config.feature_dim_s8
        )
